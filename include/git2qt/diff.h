...
public:
    ...
    TreeChanges compare(const Commit& from, const Commit& to, const CompareOptions& compareOptions, DiffModifiers diffFlags);
    TreeChanges compare_DEP(const Tree& fromTree, const Tree& toTree);
    ...
    GIT::DiffDelta::List diffTreeToTree_DEP(const Tree& oldTree, const Tree& newTree, const CompareOptions& compareOptions, DiffModifiers diffFlags = DiffModifier::DiffModNone) const;
    GIT::DiffDelta::List diffCommitToCommit(const Commit& oldCommit, const Commit& newCommit, const CompareOptions& compareOptions, DiffModifiers diffFlags = DiffModifier::DiffModNone) const;
...
private:
    ...
    DiffHandle buildDiffList(const Commit& oldCommit, const Commit& newCommit, const CompareOptions& compareOptions, DiffModifiers diffFlags = DiffModifier::DiffModNone) const;
    void detectRenames(const DiffHandle& handle, const CompareOptions& compareOptions) const;
    TreeChanges buildTreeChanges(const DiffHandle& handle) const;
